...
do
	if [ "$i" !=  "$MAX_SEGMENT" ]
	then 
		curl -L --range ${START_SEG}-${END_SEG} -o ${FILENAME}.part${i} ${URL} &> ${FILENAME}.part${i}.log &
		pids="$pids $!"
	else
		curl -L --range ${START_SEG}- -o ${FILENAME}.part${i} ${URL} &> ${FILENAME}.part${i}.log &
		pids="$pids $!"
	fi
	echo "Fetching range: ${START_SEG} - ${END_SEG}"
	START_SEG=$((${END_SEG}+1))
...
echo "Download took ${RUNTIME} seconds"
